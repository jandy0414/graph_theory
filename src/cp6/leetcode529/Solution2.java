...
//        }
    ...
    private void setE8isVisited(int v){
        int i=v/C,j=v%C;
        for(int d=0;d<8;d++){
            int nextX=i+dirs8[d][0];
            int nextY=j+dirs8[d][1];
            if(inArea(nextX,nextY) && this.needToChangeN.contains(nextX*C+nextY)){
    ...
    private HashSet<Integer>[] constructGraph(int curI){
        ...
        for(int v=0;v<g.length;v++){
            int x=v/C,y=v%C;
            if(grid[x][y]==curI && !this.needToChangeN.contains(v)){//不包含数字的E
                for(int d=0;d<4;d++){
                   int nextX=x+dirs4[d][0];
                   int nextY=y+dirs4[d][1];
                    if(inArea(nextX,nextY) && grid[nextX][nextY]==curI && !this.needToChangeN.contains(v)){//不包含数字的E
